...
```javascript copy showLineNumbers {2}
const age = 40;
age = 41;
...
2번 줄의 구문에서는 변수를 만들어주는 `const` 가 사용되지 않았으므로, `age` 라는 변수가 우리 프로그램에 이미 존재한다는 전제하에 실행됩니다.
...
하지만 우리가 [이전](../carousel-project-1/15-javascript-13.mdx)에 살펴봤다시피, `const`는 재할당이 안되기 때문에 `let`으로 변경해주어야 합니다.
...
조금 낯설어 보일 수 있지만 재미있는 연산자를 한번 살펴보겠습니다. 쓰임새가 많은 연산자입니다.
