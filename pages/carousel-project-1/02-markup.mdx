...
`ul`은 [Block 요소](../google-project-2/03-inline-block.mdx#1-1-가운데-정렬하기)이기 때문에, 너비와 높이를 지정해준 상태에서 좌우 `margin`을 `auto`로 설정하여 가로선 상에서 중앙에 배치할 수 있습니다.
...
이전 단계에서 [Position 속성](../google-project-2/05-search-input.mdx#4-css-position)에 대해 살펴본 내용에 대해 기억하고 계신가요?
...
기억하시겠지만, CSS에서는 [CSS 선택자](../google-project-2/02-css-selectors.mdx) 문법을 활용하여 우리가 원하는 요소를 지정할 수 있습니다.
