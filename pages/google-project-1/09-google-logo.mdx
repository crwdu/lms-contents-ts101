...
<Callout emoji="📌">
  ...
  아래 파일 구조를 참고하여 반드시 `index.html` 파일과 같은 경로에 저장해주세요.
</Callout>
...
문서의 흐름이 위에서 아래로 진행되기 때문에, 저는 보통 아래와 같이 페이지 구조를 수평구조로 분할하는 작업을 우선적으로 합니다. 그리고 분할한 아래의 빨간 박스들을 각각 하나의 [Block 요소](03-inline-block.md#1-block-elements)로 고려하여 작업을 진행합니다.
...
<Callout emoji="📌">
  기존에 `header` 라는 Block 요소를 활용하여 상단의 링크들을 모두 감싸놓은 것 또한 수평선
  상에서 Block 요소를 배치하고 그 내부에 링크들을 모아놓기 위함이었습니다.
</Callout>
...
<body>
  ...
  <div>
    <a>Google Search</a> <a>I'm Feeling Lucky</a>
  </div>
...
[Block 요소](03-inline-block.md#1-block-elements)인 `div` 태그를 활용하여 기본적인 구조를 잡아주었습니다. 이제 이미지의 스타일을 먼저 조정해보겠습니다.
